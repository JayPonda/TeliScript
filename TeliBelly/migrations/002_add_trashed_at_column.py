#!/usr/bin/env python3
"""
Migration: Add 'trashed_at' column to messages table
Created: 2025-12-25

This migration adds a timestamp column to the messages table:
- trashed_at: datetime, nullable (for marking when a message was trashed)
"""
...
def migrate_up(db_path: str) -> bool:
    ...
    try:
        ...
        if 'trashed_at' not in columns:
            cursor.execute("ALTER TABLE messages ADD COLUMN trashed_at TEXT")
            print("✅ Migration successful: Added 'trashed_at' column")
        else:
            print("ℹ️ Migration skipped: 'trashed_at' column already exists")
...
def migrate_down(db_path: str) -> bool:
    ...
    try:
        ...
        if 'trashed_at' not in columns:
            print("ℹ️ Rollback skipped: 'trashed_at' column doesn't exist")
            return True
        
        # SQLite doesn't support DROP COLUMN in older versions
        # We'll use a safe method: create new table, copy data, drop old, rename
        temp_columns = [col for col in columns if col != 'trashed_at']
        ...
        print("✅ Rollback successful: Removed 'trashed_at' column")
        return True
    except Exception as e:
